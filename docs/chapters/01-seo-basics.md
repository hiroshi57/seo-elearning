...
- A. 検索エンジンに広告費を払い、検索結果の上部に強制的に表示させる施策
- B. 検索エンジンがコンテンツを理解しやすくし、ユーザーが検索を通じてサイトを見つけやすくするための施策　✅**（正解）**
...
- B. 初期はWebサイトを人力で分類するディレクトリ型が使われていたが、Web の急激な増加を背景にロボット型が主流になった　✅**（正解）**
- C. 日本ではYahoo! JAPANが一度もGoogleの検索エンジンを採用したことはない
...
- B. Yahoo! JAPANもGoogleの検索エンジンを採用しているため、日本で使われている検索の多くが実質的にGoogleのアルゴリズムで処理されている　✅**（正解）**
- C. 日本の検索エンジン市場はディレクトリ型検索が現在も主流である
- D. 日本ではGoogleのシェアが0％に近く、SEOの対象にする必要はない
...
- A. SEOは出稿した瞬間に効果が出るが、広告は効果が出るまで数カ月かかる
- B. 広告は一度上位表示されると、費用をかけなくても効果が半永久的に持続する
- C. SEOは成果が出るまで時間がかかる一方、広告に比べて中長期的な費用対効果に優れる傾向がある　✅**（正解）**
...
- B. 検索エンジンのアルゴリズム変動に応じた継続的なメンテナンスが必要になる　✅**（正解）**
- C. 出稿を停止すると同時にすべての効果が消える
- D. 質の低い施策（スパム的な手法など）は評価を下げるリスクがある　✅**（正解）**
...
- B. 検索エンジンにサイトの内容を正しく理解してもらうこと
- C. 特定キーワードでの掲載順位や集客数を確約すること　✅**（正解）**
