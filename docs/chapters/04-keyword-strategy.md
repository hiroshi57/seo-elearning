...
- A. そのキーワードで上位表示されている競合サイトの数
- B. 特定のキーワードが一定期間に検索エンジンで検索された回数の目安　✅**（正解）**
- C. 検索結果ページに表示される広告枠の数
- D. そのキーワードでの上位表示の難易度をGoogleが公式に数値化した指標

**解説**：検索ボリュームは検索された回数の目安です。競合数や広告枠数ではなく、また『難易度』は各ツールの推定値でGoogle公式の数値ではありません。
...
- A. 検索意図が最も限定された、複数語の組み合わせからなるキーワード
- B. 検索ボリュームが常にゼロに近い、ニッチなキーワード
- C. 自社では対策せず、競合の動向確認だけに使うキーワード
- D. 自社にとって重要かつ関連性が高い、検索意図の幅が広い単一のキーワード　✅**（正解）**

**解説**：軸キーワードは検索意図の幅が広い単一のキーワードです。複数語で意図が限定されたものはロングテール寄りで、軸キーワードとは異なります。

### Q4（複数選択）

軸キーワードからキーワードを広げる方法として、正しいものをすべて選べ。

- A. サジェストキーワード（オートコンプリート）を収集する　✅**（正解）**
- B. Googleキーワードプランナーなどの関連キーワードツールを活用する　✅**（正解）**
- C. 自社サイトの流入キーワードや競合サイトの対策キーワードを確認する　✅**（正解）**
- D. 軸キーワードと関係のないキーワードだけをあえて優先的に集める
...
- B. 検索ボリュームの大きい順に並べれば十分なので、実際には不要な作業である
- C. 検索エンジンに大量のキーワードを送信し、インデックスを促進するため
- D. 競合サイトのキーワードを漏れなく自社のタイトルに含めるため

**解説**：グルーピングは同じニーズの表現を束ね、ニーズの大きさを正しく捉えるための作業です。単純なボリューム順では同一ニーズが分散し、正しく把握できません。
...
- A. ピラーページはトピック全体を網羅的に扱う中心的なページである　✅**（正解）**
- B. クラスターページはピラーページの中の具体的なサブテーマを扱う　✅**（正解）**
- C. ピラーページとクラスターページは内部リンクでつなげる　✅**（正解）**
- D. クラスターページはピラーページと一切関連付けてはならない
...
- A. 1語のみで構成され、検索ボリュームが非常に大きいキーワード
- B. 検索ボリュームが大きいわりに、競合が常に存在しないキーワード
- C. 2語・3語以上を組み合わせた具体的なキーワードで、検索意図が明確な傾向がある　✅**（正解）**
- D. 必ず商品名だけで構成される、指名検索のキーワード

**解説**：ロングテールは複数語からなる具体的なキーワードで、ボリュームは小さいものの意図が明確で競合も比較的弱い傾向があります。

### Q9（単一選択）

SEOにおける「競合」の考え方として、最も適切なものはどれか。

- A. 自社と同じ業界・業種の企業だけが、常に競合になる
- B. 検索結果に表示されていないサイトほど、警戒すべき強い競合である
- C. 競合分析は不要で、自社の施策だけを考えれば十分である
- D. 対策したいキーワードで実際に上位表示されているサイトを競合として捉える　✅**（正解）**

**解説**：SEOの競合は『対策キーワードで実際に上位表示されているサイト』です。比較サイトやメディアなど、業種の異なるサイトが競合になることもあります。

### Q10（単一選択）

Googleのsite:コマンドに関する説明として、最も適切なものはどれか。

- A. site:ドメインの形式で検索し、そのドメインのインデックス状況やおおよその規模を把握する手がかりになる　✅**（正解）**
- B. そのドメインの正確なインデックス数と検索順位を確定的に取得できる
- C. リスティング広告の入札額や品質スコアを調べるための機能である
- D. 有料のSEOツールを契約しないと使えない、Googleの専用機能である

**解説**：site:コマンドはインデックス状況や規模感を推測する手がかりで、正確な件数や順位が分かるわけではありません。無料で使え、広告の入札額とは無関係です。
