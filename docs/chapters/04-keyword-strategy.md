...
- A. 特定のキーワードが一定期間に検索された回数の目安　✅**（正解）**
- B. そのキーワードで上位表示されているサイトの数
- C. 検索結果ページに表示される広告の数
- D. サイトの表示速度を数値化した指標
...
- A. 自社にとって重要かつ関連性が最も高い、検索意図の幅が広い単一のキーワード　✅**（正解）**
- B. 検索意図が最も限定された、複数語からなるキーワード
- C. 競合サイトだけが使用しているキーワード
- D. 検索ボリュームが常にゼロのキーワード
...
- A. 検索ボリュームが大きい順にキーワードを並べるだけで十分なので、グルーピングは不要である
- B. 同じニーズを表すキーワード同士をまとめ、ニーズごとの大きさを正しく把握するため　✅**（正解）**
...
- B. 2語・3語以上を組み合わせた具体的なキーワードで、検索意図が明確な傾向がある　✅**（正解）**
- C. 検索エンジンには一切認識されないキーワード
...
- B. 対策したいキーワードで実際に上位表示されているサイトを競合として捉える　✅**（正解）**
- C. 競合は分析する必要がなく、自社の施策だけを考えればよい
- D. 検索結果に表示されないサイトほど強い競合である
