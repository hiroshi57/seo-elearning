...
- A. 広告収益を最大化するエンジン
- B. ユーザーの意図を正確に把握し、ユーザーのニーズに一致するものを返すエンジン　✅**（正解）**
- C. 最も文字数の多いページを上位表示するエンジン
- D. 検索結果を表示せず、常に1つの答えだけを提示するエンジン
...
- A. 特定の1サイトをターゲットにしてGoogleが行う個別のペナルティ処置
- B. 検索結果全体の有用性・信頼性を高めることを目的とした、広範なアルゴリズム変更　✅**（正解）**
...
- A. 検索するユーザーにとって本当に役立つ内容になっているか　✅**（正解）**
- B. 専門知識を持つ人物・情報源によって作られているか　✅**（正解）**
- C. 他のページには無い、独自の視点・データ・分析が含まれているか　✅**（正解）**
- D. 検索エンジン向けにキーワードをできる限り多く詰め込んでいるか
...
- B. 外部の評価者向けにGoogleが運用している内部マニュアルであり、Googleが重視する品質の基準を知る手がかりになる　✅**（正解）**
- C. サイト運営者が自分のサイトを自己採点するための公式スコアリングツールである
...
- B. 独自の視点・データ・分析を含むコンテンツは、品質評価において重視される傾向にある　✅**（正解）**
- C. 独自性は検索順位に一切関係しない
- D. 文章の長さだけが独自性の判断基準になる
...
- A. 他サイトから言及される回数が多ければ多いほど良い
- B. どのような文脈・信頼性を持つサイトから参照されているかという質的な側面　✅**（正解）**
