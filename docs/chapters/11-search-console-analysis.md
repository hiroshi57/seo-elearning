...
- A. クリック数をセッション数で割った値
- B. クリック数を表示回数で割った、クリックされた割合　✅**（正解）**
- C. 表示回数をクリック数で割った値
- D. クリックからCV（コンバージョン）に至った割合

**解説**：CTR＝クリック数÷表示回数です。分母・分子を逆にした値や、CVR（CVに至った割合）と混同しないよう注意します。
...
- A. そのページをrobots.txtでブロックし、表示自体を止める
- B. 対象キーワードを本文から削除して表示回数を減らす
- C. サーバーのスペックを上げて表示速度を改善する
- D. タイトル・ディスクリプション（スニペット）を見直し、クリックされやすくする　✅**（正解）**

**解説**：「表示は多いがCTRが低い」は、スニペットの魅力度に改善余地があるサインです。ブロックやキーワード削除、速度改善はCTR低下の直接的な対策になりません。

### Q5（単一選択）

「検出 - インデックス未登録」の状態の説明として、最も適切なものはどれか。

- A. ページの存在は発見されたが、まだクロールされていない状態　✅**（正解）**
- B. クロールは済んだが、低品質等の理由でインデックスされていない状態
- C. robots.txtでクロールを明示的に禁止している状態
- D. 一度インデックスされたが、後から削除された状態

**解説**：「検出 - インデックス未登録」は発見済みだが未クロールの状態です。『クロール済み - インデックス未登録』やrobots.txtブロックなど、似た状態との区別が重要です。
