...
- A. サイト訪問後のユーザー行動だけを計測するツール
- B. Googleが提供する、自サイトが検索結果でどう扱われているかを確認・管理できる無料ツール　✅**（正解）**
- C. 有料広告の入札単価を自動調整するツール
- D. サイトのデザインテンプレートを提供するツール

**解説**：Search Consoleは、自分のサイトがGoogle検索でどのように扱われているかを確認・管理できる、Googleが提供する無料のツールです。
...
- A. 特定のURL配下のみを対象とする
- B. サブドメインを含むドメイン全体を対象とし、http/https/wwwの有無を問わずまとめて扱う　✅**（正解）**
- C. 所有権確認の方法はHTMLファイルのアップロードのみである
- D. ドメインプロパティは現在Googleでは利用できない
...
- A. 所有権確認をしなくても、すべての機能をすぐに使うことができる
- B. 所有権が確認されるまでは、詳細な情報の閲覧や各種機能の利用ができない　✅**（正解）**
...
- B. サイトの全容を把握することが、場当たり的でない改善施策を考えるための土台になる　✅**（正解）**
- C. URLは一度整理したら二度と確認する必要がない
...
- A. サイトのデザインカラーが競合と似ていないか
- B. robots.txtで重要ページが誤ってブロックされていないか、404・500エラーが多発していないか　✅**（正解）**
- C. 従業員の人数が十分かどうか
- D. SNSのフォロワー数が増えているかどうか

**解説**：クロール健全性の確認では、robots.txtによる意図しないブロック、noindexの誤設定、404・500エラーの多発、クロール頻度の急激な増減などを確認します。

### Q9（単一選択）

レンダリングの健全性を確認する方法として、最も適切なものはどれか。

- A. URL検査ツールで「クロール済みページを表示」し、Googlebotが解釈したHTMLやスクリーンショットを確認する　✅**（正解）**
- B. GA4の売上レポートを確認する
- C. 競合サイトのSNSフォロワー数を確認する
- D. 自社のメールマガジン開封率を確認する

**解説**：URL検査ツールの「クロール済みページを表示」から、Googlebotが解釈したHTMLソースやレンダリング結果のスクリーンショットを確認し、JavaScriptで生成されるコンテンツが正しく認識されているかなどをチェックできます。
