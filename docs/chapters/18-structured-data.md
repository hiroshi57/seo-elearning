...
- B. 両者は全く同じ役割を持ち、区別する必要はない
- C. 構造化タグも構造化マークアップも、ブラウザの見た目を大きく変化させる
- D. 構造化タグは文書の構造を伝え、構造化マークアップは要素の意味を伝える　✅**（正解）**
...
- A. タグには文書構造の明示や意味付けの役割はなく、機械的な置換はできない　✅**（正解）**
- B. 既存のタグはそのまま構造化タグに置き換えられる
...
- A. ページの見た目（レイアウト・装飾）を制御するためのマークアップ
- B. ページ内の情報の「意味」を、検索エンジンにも分かる形式で明示するマークアップ　✅**（正解）**
- C. ページの表示速度を最適化するためのメタ情報
- D. クローラーの巡回頻度を指定するための記述

**解説**：構造化データは情報の「意味」を検索エンジンに伝えるマークアップです。見た目の制御（CSS）や速度最適化、クロール頻度指定とは目的が異なります。
...
- B. XHTMLでしか利用できない
- C. 検索エンジンにまったく読み取られない
- D. 可視データについては、HTML本文とJSON-LDの2箇所に同じ内容を記述する必要がある　✅**（正解）**

**解説**：JSON-LDは本文と分離して書ける利点がある一方、可視データは本文とJSON-LDの二重管理になる点がデメリットです。属性増加はMicrodata/RDFaの特徴です。
...
- A. 評価対象となる商品・店舗の構造化マークアップと1対1のセットで記載し、ページ本文にも必ず表示する　✅**（正解）**
- B. カテゴリ単位でまとめて評価をマークアップするのが望ましい
- C. ページ本文に表示されていない評価も自由にマークアップしてよい
...
- A. Articleタイプを記述すると、必ず1位に表示されるため
- B. 著者情報や公開日を明示することで、検索エンジンがコンテンツの専門性や鮮度を認識しやすくなるため　✅**（正解）**
...
- A. Search Consoleの拡張レポート　✅**（正解）**
- B. スキーマ マークアップ検証ツール　✅**（正解）**
- C. リッチリザルト テスト　✅**（正解）**
- D. Googleキーワードプランナー
...
- B. 構造化データの実装効果について、Googleは一切事例を公開していない
- C. Rotten Tomatoesは構造化データを実装したページでクリック率が25%増加した　✅**（正解）**
