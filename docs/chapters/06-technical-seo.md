...
- B. 抜け漏れさえなければ、カテゴリ同士が多少重複していても問題ない
- C. 同じページを、関連する複数のカテゴリに重複して登録すること
- D. カテゴリ分けはせず、全ページをフラットに並べること

**解説**：MECEは『重複なく・漏れなく』分類することです。重複を許容したり、意図的に重複登録したりする状態はMECEではありません。
...
- A. title要素はページ内容を端的に表し、サイト内でユニークな内容にすることが望ましい　✅**（正解）**
- B. meta descriptionは検索結果のスニペットに使われることがあり、ページごとにユニークな記述が推奨される　✅**（正解）**
- C. h要素は階層（h1→h2→h3…）を飛ばさず、装飾目的だけで使うべきではない　✅**（正解）**
...
- A. 検索条件が異なっても、常に同一URL（例：search.php）で結果が表示される状態　✅**（正解）**
- B. 1つのURLに1つのまとまったコンテンツが対応している状態
- C. 各ページにユニークなtitle・descriptionが設定されている状態
- D. ページネーションの各ページが自己参照のcanonicalを持つ状態

**解説**：検索条件が違うのに同じURLになる状態は『1URL1コンテンツ』が崩れた例です。B・C・Dはいずれも原則が適切に守られている状態です。
...
- B. canonicalタグは、ユーザーを転送せずに正規URLを検索エンジンへ伝える　✅**（正解）**
- C. Googleは基本的に301リダイレクトを推奨しており、できない場合の代替としてcanonicalを案内している　✅**（正解）**
...
**解説**：課題を頂点に置き、その原因をMECEに分解するのはWHYツリーです。WHATは構成要素の分解、HOWは手段の分解に用います。
...
- B. 関連するページ・上位カテゴリへリンクを張る　✅**（正解）**
- C. 常にトップページ・最上位カテゴリへ戻れるリンクを用意する　✅**（正解）**
...
- B. パンくずリスト内のテキストリンク
- C. href属性を持たず、JavaScriptのonclickだけで遷移するリンク　✅**（正解）**
- D. グローバルナビゲーション内のテキストリンク

**解説**：クローラは基本的にhref属性をたどるため、href無しでonclickだけのリンクは辿れないことがあります。通常のaタグ・パンくず・ナビは問題になりにくいリンク形式です。
...
- A. そのページの表示速度が自動的に向上する
- B. そのページがインデックスに登録されないようになる　✅**（正解）**
- C. そのページの検索順位が一定期間だけ上がる
- D. そのページから他サイトへの外部リンクがすべて無効になる

**解説**：meta robotsのnoindexは、そのページをインデックス対象から外す指定です。表示速度や順位、外部リンクの有効性を変えるものではありません。
...
- A. 404エラーページをまったく用意していない状態
- B. 404ページのデザインをサイトの他ページと統一した状態
- C. リンク切れを自動で検出・修復できるツールのこと
- D. 存在しないページなのに、ステータスコード200（正常）を返してしまっている状態　✅**（正解）**

**解説**：ソフト404は、実質存在しないページが200を返している状態を指します。本来は404（または410）を返すべきです。デザイン統一やツールのことではありません。
...
- A. ネットワーク処理（ブラウザとサーバー間のデータの送受信）　✅**（正解）**
- B. サーバー処理（リクエストを受けてHTMLを組み立てる）　✅**（正解）**
- C. レンダリング処理（受け取ったデータを画面に描画する）　✅**（正解）**
- D. インデックス処理（クロールしたページをデータベースに登録する）
...
- A. 1つのTCPコネクション内で複数のストリームを多重化し、順不同で処理できる　✅**（正解）**
- B. 1つのコネクションで、1組のリクエストとレスポンスしか処理できない
- C. 常に非暗号化（平文HTTP）での通信が前提となっている
- D. 画像を軽量化するための新しい圧縮フォーマットである

**解説**：HTTP/2は多重化により1コネクションで複数リクエストを並行処理できます。1組ずつしか扱えないのはHTTP/1.1で、画像圧縮形式でもありません。

### Q20（単一選択）

キャッシュに関する説明として、最も適切なものはどれか。

- A. 一度設定すると、ファイルの中身を二度と更新できなくなる
- B. 保存場所によってブラウザキャッシュとサーバーキャッシュの2種類がある　✅**（正解）**
- C. 更新頻度が高いファイルほど、長期間のキャッシュを設定すべきである
- D. キャッシュはネットワーク処理やレンダリングとは無関係な仕組みである

**解説**：キャッシュには保存場所によりブラウザ／サーバーの2種類があります。更新頻度の高いファイルに長期キャッシュは不向きで、表示速度（ネットワーク処理）に関わります。

### Q21（複数選択）

Core Web Vitalsを構成する3つの指標として、正しいものをすべて選べ。

- A. LCP（Largest Contentful Paint）：読み込み速度　✅**（正解）**
- B. INP（Interaction to Next Paint）：操作への応答性　✅**（正解）**
- C. CLS（Cumulative Layout Shift）：視覚的な安定性　✅**（正解）**
- D. CTR（Click Through Rate）：クリック率
...
- A. インパクトの割合と距離の割合を足し合わせた値がスコアになる
- B. 要素が移動したピクセル数そのものがスコアになる
- C. ページの総読み込み時間からスコアが自動算出される
- D. インパクトの割合と距離の割合を掛け合わせた値がスコアになる　✅**（正解）**

**解説**：CLSは『インパクト率 × 距離率』で算出します。加算やピクセル数そのもの、読み込み時間からの算出ではありません。

### Q24（単一選択）

Search Console・Lighthouse・PageSpeed Insightsの違いに関する説明として、最も適切なものはどれか。

- A. Lighthouseは非公開ページも調査でき、実行端末の環境に依存するラボデータを提供する　✅**（正解）**
- B. 3つのツールはすべて同一のデータソース・同一の対象ページを扱う
- C. Search Consoleは非公開ページの調査に特化したツールである
- D. PageSpeed Insightsはフィールドデータもラボデータも一切扱えない

**解説**：Lighthouseはローカル実行でき非公開ページも測定可（ラボデータ）。Search Consoleは実ユーザーのフィールドデータ中心で公開ページ対象、PageSpeed Insightsは両データを扱えます。

### Q25（単一選択）

HTTPS化とGoogleのランキングシグナルの関係について、公式ブログが示している見解として、最も適切なものはどれか。

- A. 検索順位にはまったく影響しないと明言されている
- B. ランキングシグナルとして使われているが、コンテンツ品質など他要素と比べ影響は軽微とされる　✅**（正解）**
- C. HTTPS化さえすれば、他の要素に関わらず必ず上位になれる
- D. セキュリティ上の意味だけで、SEO・ランキングとは無関係である

**解説**：HTTPSは軽微ながらランキングシグナルとされています。影響ゼロでも、単独で上位化を保証するものでもありません。
