...
- A. カテゴリ同士が重複していても、抜け漏れさえなければよい
- B. 重複がなく、抜け漏れもない状態でカテゴリを分類すること　✅**（正解）**
...
- A. 公開・インデックス済みのページのURLは、内容が存在する限りできるだけ変更しない　✅**（正解）**
- B. URLはSEO評価のリセットのために定期的に変更すべきである
...
- B. 旧URLから新URLへ301リダイレクトを設定し、評価をできる限り引き継ぐ　✅**（正解）**
- C. URLを変更する際は、必ずページも削除しなければならない
...
- B. 画像が表示されない場合の代替テキストであり、検索エンジンが画像内容を理解する手がかりにもなる　✅**（正解）**
- C. 画像のファイルサイズを圧縮するための属性である
- D. リンク先のURLを指定するための属性である
...
- A. 1つのURLに1つのまとまったコンテンツが対応している状態
- B. 検索条件が異なるにもかかわらず、常に同一のURL（例：search.php）で結果が表示される状態　✅**（正解）**
...
- B. href属性を伴わない、JavaScriptのonclickだけで遷移するリンク　✅**（正解）**
- C. パンくずリストのリンク
- D. グローバルナビゲーションのリンク
...
- B. canonicalで指定した正規URLのみを含める　✅**（正解）**
- C. アクセス数が少ないURLだけを含める
- D. XMLサイトマップにはURLを1つも含めない方がよい
...
- A. 1ファイルに含められるURLは無制限である
- B. 1ファイルのURL数は50,000個以下、ファイルサイズは50MB以下（圧縮前）に収める必要がある　✅**（正解）**
...
- B. PDFや画像などmeta robotsを記述できない非HTMLファイルに対して、HTTPヘッダーでクロール・インデックスを制御する仕組み　✅**（正解）**
- C. クロールバジェットを自動的に増やすための申請フォーム
- D. XMLサイトマップの別の呼び方
...
- A. Search ConsoleのURL検査ツールで、個別にクロールをリクエストする　✅**（正解）**
- B. RSS/Atomフィードで更新ページの情報を配信する　✅**（正解）**
- C. WebSubでハブ経由の更新通知を行う　✅**（正解）**
...
- B. 存在しないページに対して、ステータスコード200を返してしまっている状態　✅**（正解）**
- C. 404エラーページのデザインをサイトの他ページと統一した状態
- D. リンク切れを完全に検出できるツールのこと
...
- B. 質の低い被リンク（リンクの売買など）　✅**（正解）**
- C. クローラへの偽装行為（隠しテキスト、クローキングなど）　✅**（正解）**
- D. 画像にalt属性を正しく設定する行為
...
- A. キャッシュには保存場所によってブラウザキャッシュとサーバーキャッシュの2種類がある　✅**（正解）**
- B. キャッシュを設定すると、常にファイルの中身を書き換えずに配信し続けなければならない
...
- B. 2024年3月に、それまで使われていたFID（初回入力遅延）がINPに置き換えられた　✅**（正解）**
- C. LCPは2024年に廃止され、現在は使われていない
...
- C. インパクトの割合と距離の割合を掛け合わせた値がCLSスコアになる　✅**（正解）**
- D. CLSスコアはページの読み込み時間から自動的に算出される
...
- A. 3つのツールはすべて同じデータソース・同じ調査対象ページを扱う
- B. Lighthouseは非公開ページを含めて調査でき、端末環境に依存するラボデータを提供する　✅**（正解）**
