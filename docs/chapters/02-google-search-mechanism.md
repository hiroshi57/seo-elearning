...
- A. 検索エンジンのロボットがリンクやサイトマップをたどってWebページを発見・巡回し、内容を収集するプロセス　✅**（正解）**
- B. 収集済みのページ内容を解析し、検索用データベースに登録・整理するプロセス
- C. 検索クエリとの関連性を計算し、表示する順位を決めるプロセス
- D. 取得できなかったURLに対して、代替ページを自動生成するプロセス

**解説**：クローリングはロボットがページを発見・巡回して収集する工程です。B（登録・整理）はインデックス、C（順位付け）はランキングの説明で、混同しやすい選択肢です。

### Q2（単一選択）

「クロールはされたが、インデックスはされていない」ページについての説明として、正しいものはどれか。

- A. クロール済みであれば、インデックス前でも検索結果に表示される
- B. インデックスされていないため、検索結果には表示されない　✅**（正解）**
- C. クロールとインデックスは同義のため、この状態は発生しない
- D. インデックス前のページは、インデックス済みページより上位に表示される

**解説**：検索結果に表示されるにはインデックス登録が必要です。クロール（発見）とインデックス（登録）は別工程で、クロールされただけでは表示されません。

### Q3（単一選択）

Googleのランキングの仕組みに関する説明として、最も適切なものはどれか。

- A. 被リンクの本数が最も多いページが、内容にかかわらず常に最上位になる
- B. 本文の文字数が多いページほど、機械的に上位に表示される
- C. 関連性・品質・有用性など、数多くのシグナルを組み合わせて総合的に順位が決まる　✅**（正解）**
- D. 更新日時が最も新しいページが、内容にかかわらず常に上位になる

**解説**：ランキングは単一の指標ではなく、多数のシグナルの組み合わせで決まります。被リンク数・文字数・更新日時などの単一要素だけで順位が決まるわけではありません。
...
- B. BERT　✅**（正解）**
- C. RankBrain　✅**（正解）**
- D. PageRank（リンクによる評価を扱う仕組み）

**解説**：RankBrain・BERT・ニューラルマッチングは検索クエリの意図・文脈理解に関わる仕組みです。PageRankはリンクによる評価の仕組みで、意図理解のシステムではありません。
...
**解説**：一般に使われる分類はKnow・Do・Go・Buyの4つです。『Sell（売りたい）』は含まれません（Buyとの取り違えに注意）。
...
- A. Website queryは特定の1サイトを、Visit-in-Person queryは近くの特定ジャンルの複数店舗・施設を探している　✅**（正解）**
- B. Website queryは公式サイトを、Visit-in-Person queryはSNSアカウントを探すクエリである
- C. Visit-in-Person queryはモバイル検索専用で、Website queryはPC検索専用である
- D. Visit-in-Person queryは購入意欲が最も高いBuyクエリと同じ意味である

**解説**：Website queryは1つに特定されるサイト、Visit-in-Person queryは近隣の複数店舗・施設を探すクエリです。デバイス限定やBuyと同義といった説明は誤りです。

### Q8（単一選択）

ゼロクリック検索（Zero-click Search）の説明として、最も適切なものはどれか。

- A. 検索結果にオーガニックのページが1件も表示されない状態のこと
- B. 検索結果ページ上で疑問が解決し、ユーザーがどのサイトもクリックしないまま検索が完結すること　✅**（正解）**
- C. 広告を避けてオーガニック検索結果だけをクリックするユーザー行動のこと
- D. クリック課金広告で、クリックが発生せず費用がかからない状態のこと

**解説**：ゼロクリック検索は、強調スニペットやAI Overviewなどで検索結果画面上だけで疑問が解決し、サイトに遷移しないまま完結する現象を指します。
