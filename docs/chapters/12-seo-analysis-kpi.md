...
- A. KPIが最終目標で、KGIはその達成度を測る補助指標である
- B. KGIは日次で追う指標、KPIは四半期で追う指標という時間軸の違いである
- C. KGIは最終的な目標を示し、KPIはKGIを達成するための中間指標である　✅**（正解）**
- D. KGIは定量指標、KPIは定性指標という測定方法の違いである

**解説**：KGI（最終目標）とKPI（中間指標）の関係です。両者の役割を逆にしたり、時間軸・定量定性の違いと説明したりするのは誤りです。
...
- B. KPIは一度設定したら、事業フェーズが変わっても変更してはならない
- C. KPIはGA4やSearch Consoleとは無関係に決めるべきである
- D. 主要なKPIを3〜5個程度に絞り込むことで、注目すべき変化が見えやすくなる　✅**（正解）**
...
- A. 集客の立ち上げ期は流入数や掲載順位、一定の集客確保後はCV数・CV率が重視されやすい　✅**（正解）**
- B. 集客の立ち上げ期でも、一定の集客確保後でも、重視すべきKPIは常に同じである
...
**解説**：Search Consoleの標準データ保持期間は16か月です。長期比較にはBigQueryなどへのエクスポートが必要になります。

### Q6（単一選択）

Search Consoleの「一括データエクスポート」機能でBigQueryにデータを送る利点として、最も適切なものはどれか。

- A. エクスポートできる行数が最大1,000行に制限される
- B. 送信するとSearch Consoleの保持期間が3か月に短縮される
- C. 行数無制限でデータを蓄積でき、GA4など他データとの統合分析もしやすくなる　✅**（正解）**
- D. 送信後はSearch Consoleの管理画面が使えなくなる

**解説**：BigQueryへの一括エクスポートは行数無制限で蓄積でき、他データとの統合分析に向きます。行数1,000行制限は管理画面の制約です。

### Q7（単一選択）

被リンクのモニタリングに関する説明として、最も適切なものはどれか。

- A. 被リンクは一度獲得すれば価値が永久に保証されるため、確認は不要である
- B. 被リンクの確認はGA4のトラフィック獲得レポートで行う
- C. 被リンクは内部リンクの一種で、サイト内だけで完結する
- D. 被リンクは現時点の外部評価を映す指標で、失われることもあるため継続的な確認が必要　✅**（正解）**

**解説**：被リンクは蓄積される固定資産ではなく、現時点の外部評価を映す指標です。失われることもあるため継続的なモニタリングが必要です。

### Q8（単一選択）

順位や流入の異常を早期に検知する仕組みとして、最も適切なものはどれか。

- A. GA4のカスタムインサイトやLooker Studioのアラート機能を使い、条件に合致した際に通知を受け取る　✅**（正解）**
- B. 毎日手作業ですべてのページを目視確認する以外に方法はない
